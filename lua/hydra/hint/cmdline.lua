...
function HintAutoCmdline:_make_message()
  ...
  for _, head_spec in ipairs(heads) do
    ...
    if desc ~= false then
      space = self:_add_chunk(
        hint,
        space,
        { head_spec.head, "Hydra" .. head_spec.color }
      )
      if not space then
        break
      end
    ...
    while start do
      ...
      if start then
        ...
        if not fun then
          error(
            string.format(
              '[Hydra] "%s" not present in "config.hint.functions" table',
              found
            )
          )
        end
    ...
    while start do
      ...
      if start then
        if not heads[found] then
          error(
            string.format(
              '[Hydra] docsting error, head "%s" does not exist',
              found
            )
          )
        end
