...
function Hydra:initialize(input)
      ...
      vim.validate({
        foreign_keys = {
          ...
          function(foreign_keys)
            if
              type(foreign_keys) == "nil"
              or foreign_keys == "warn"
              or foreign_keys == "run"
            then
              return true
            else
  ...
  do
    ...
    if
      self.config.color
      ~= util.get_color_from_config(self.config.foreign_keys, self.config.exit)
    then
  ...
  local on_enter = self.config.on_enter
  self.config.on_enter = function(...)
...
function Hydra:_setup_hydra_keymaps()
  ...
  for head, map in pairs(self.heads) do
    ...
    if
      not self.config.invoke_on_body
      and not opts.exit
...
function Hydra:_setup_pink_hydra()
  ...
  for head, map in pairs(self.heads) do
    ...
    table.insert(
      opts.exit and layer.exit or layer.layer,
      { mode, head, rhs, o }
    )
  end
